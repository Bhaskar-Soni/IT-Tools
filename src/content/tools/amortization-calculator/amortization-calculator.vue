...
const loanAmountValidation = useValidation({
  ...
  rules: [
    {
      message: "Loan amount must be a positive value",
...
const interestRateValidation = useValidation({
  ...
  rules: [
    {
      message: "Interest rate must be a positive value between 0 and 100",
...
<template>
  <c-card title="Loan Parameters" mb-5>
    <c-input-text
    ...
    <c-select v-model:value="selectedCurrency" :options="supportedCurrencies" label="Displayed Currency" mb-5 />
  </c-card>
    ...
    <n-table>
      ...
      <tbody>
        <tr v-for="period in amortizationSchedule" :key="period.type + period.paymentIndex">
          ...
          <template v-else>
            <th>End of Year {{ period.paymentIndex }}</th>
