    ...
    <c-card title="Dates Interval" mb-2>
      ...
      <div mb-2 flex items-baseline gap-2>
        <n-form-item label="Business Start Hour:" label-placement="left" flex-1>
          <NInputNumber v-model:value="businessStartHour" :min="0" :max="24" />
        </n-form-item>
        <n-form-item label="Business End Hour:" label-placement="left" flex-1>
          <NInputNumber v-model:value="businessEndHour" :min="0" :max="24" />
        </n-form-item>
      </div>
      ...
      <c-card v-if="resultDaysDiff" title="Result">
        <input-copyable v-bind="inputProps" label="Start Date" :value="resultDaysDiff.startDate.toString()" />
        <input-copyable v-bind="inputProps" label="Start Date (ISO)" :value="resultDaysDiff.startDate.toISOString()" />
        <input-copyable v-bind="inputProps" label="End Date" :value="resultDaysDiff.endDate.toString()" />
        ...
        <n-divider />
        <input-copyable v-bind="inputProps" placeholder="None" label="Mondays" :value="resultDaysDiff.mondays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Tuesdays" :value="resultDaysDiff.tuesdays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Wednesdays" :value="resultDaysDiff.wednesdays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Thursdays" :value="resultDaysDiff.thursdays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Fridays" :value="resultDaysDiff.fridays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Saturdays" :value="resultDaysDiff.saturdays.join(', ')" />
        <input-copyable v-bind="inputProps" placeholder="None" label="Sundays" :value="resultDaysDiff.sundays.join(', ')" />
        <n-divider />
