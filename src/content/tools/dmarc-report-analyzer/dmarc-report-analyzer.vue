...
const parsedReport = computedAsync(async () => {
  ...
  error.value = '';
  try {
    const parserModule = await import('dmarc-report-parser');
    const { parseDmarcReportFromEmail, parseDmarcReportsFromXml } = parserModule as typeof import('dmarc-report-parser');
