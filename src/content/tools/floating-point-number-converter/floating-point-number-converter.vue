...
const binaryOutput = computed(() => {
  try {
    return convertDecimalToBinary({ value: decimalInput.value, bitCount: bitCount.value });
  }
  catch {
    return '';
  }
...
const binaryFromDecimal = computed(() => {
  try {
    return convertBase({ value: binaryOutput.value, fromBase: 2, toBase: 16 });
  }
  catch {
    return '';
  }
...
const actualDecimalValue = computed(() => {
  try {
    return errorlessBinaryToDecimalConversion({ value: binaryOutput.value, decimalPrecision: '32', removeZeroPadding: true });
  }
  catch {
    return '';
  }
...
<template>
  <c-card title="Decimal to Binary" style="min-width: 650px">
    ...
    <InputCopyable
      ...
      :value="calcErrorDueToConversion({ decimalInput: decimalInput.value, actualValue: actualDecimalValue.value })"
      readonly
      label-position="left"
      label-width="210px"
      label-align="right"
  ...
  <c-card title="Binary to Decimal" style="min-width: 650px">
    ...
    <InputCopyable
      ...
      :value="errorlessBinaryToDecimalConversion({ value: binaryInput.value, decimalPrecision: decimalPrecision.value, removeZeroPadding: !showTrailingZeros.value })"
      readonly
      label-position="left"
      label-width="140px"
      label-align="right"
