...
// Lazy load msgreader to handle CommonJS/ESM compatibility issues
let MsgReader: any = null;
let getProps: any = null;
let deEncapsulateSync: any = null;
let decompressRTF: any = null;
let iconv: any = null;

const loadLibraries = async () => {
  if (MsgReader) return; // Already loaded
  
  try {
    const msgReaderModule = await import('@kenjiuno/msgreader');
    MsgReader = msgReaderModule.default || msgReaderModule;
    
    const defsModule = await import('@kenjiuno/msgreader/lib/Defs.js');
    getProps = defsModule.getProps;
    
    const rtfModule = await import('rtf-stream-parser');
    deEncapsulateSync = rtfModule.deEncapsulateSync;
    
    const decompressModule = await import('@kenjiuno/decompressrtf');
    decompressRTF = decompressModule.decompressRTF;
...
function getPropertyName(tag: string | undefined) {
  ...
  const prop = getProps?.()?.find((p: any) => p.key?.toLocaleLowerCase() === tagLower);
  if (!prop) {
    return tag;
  }
