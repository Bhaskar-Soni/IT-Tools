...
const inputElement = ref<InstanceType<typeof CInputText> | null>(null);
const markdown = ref('');

// Subscribe the behavior to the textarea.
onMounted(() => {
  nextTick(() => {
    const wrapper = inputElement.value?.inputWrapperRef as HTMLElement | undefined;
    const textarea = wrapper?.querySelector('textarea');
    if (textarea) {
      subscribe(textarea as never);
    }
  });
});
</script>
