...
// import { useIDBKeyval } from '@vueuse/integrations/useIDBKeyval'; // TODO: Fix subpath import
import { extractTextFromEPUB, extractTextFromPDF } from './rsvp-reader.service';
import { useQueryParamOrStorage } from '@/composable/queryParams';

const message = useMessage();

const rawText = useQueryParamOrStorage({ name: 'text', storageName: 'rsvp:txt', defaultValue: '' });
...
watch(words, (newWords) => {
  ...
  if (currentIndex.value >= newWords.length) {
    currentIndex.value = newWords.length - 1;
  }
});
</script>
...
<template>
  <div>
    <n-tabs type="line">
      <n-tab-pane name="display" tab="Display" display-directive="show">
      ...
      <n-tab-pane name="text" tab="Text" display-directive="show">
        <c-input-text
      ...
      <n-tab-pane name="keyboard" tab="Keyboard Shortcuts" display-directive="show">
