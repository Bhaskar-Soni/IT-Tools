...
const fileInput = ref() as Ref<ArrayBuffer | undefined>;
const passphrase = ref('');
const inputType = ref<'file' | 'content'>('file');

async function onUpload(file: File) {
  if (file) {
    fileInput.value = await file.arrayBuffer();
...
const convertedCertificates = computedAsync(async () => {
  const inputContent = inputKeyOrCertificate.value;
  const file = fileInput.value;
  let inputKeyOrCertificateValue: string | ArrayBuffer | undefined = undefined;
  ...
  if (!inputKeyOrCertificateValue) return null;
  return await convertCertificates(inputKeyOrCertificateValue, passphrase.value);
});
</script>
