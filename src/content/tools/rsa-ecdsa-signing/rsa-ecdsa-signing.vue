...
const [verifyOutput, verifyError] = computedCatchAsync(async () => {
  const runningInBrowser = typeof window !== 'undefined' && typeof window.document !== 'undefined';
  if (runningInBrowser) throw new Error('SSH signing/verifying requires server-side support (sshpk).');
  const mod = await import('sshpk');
  const publicKey = (mod as any).parseKey(verifyPublicKey.value, 'auto');
...
const [signOutput, signError] = computedCatchAsync(async () => {
  const runningInBrowser = typeof window !== 'undefined' && typeof window.document !== 'undefined';
  if (runningInBrowser) throw new Error('SSH signing/verifying requires server-side support (sshpk).');
  const mod = await import('sshpk');
  const privateKey = (mod as any).parsePrivateKey(signPrivateKey.value, 'auto', { passphrase: signPrivateKeyPassphrase.value });
  const s = privateKey.createSign(signHashType.value as any);
