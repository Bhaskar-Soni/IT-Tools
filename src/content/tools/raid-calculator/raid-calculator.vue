  ...
  <div>
    <c-card>
      <n-form-item label="Number of disks" label-placement="left" label-width="150" mb-2>
        <NInputNumber v-model:value="diskTotal" max="10000" min="2" placeholder="Number of disks (ex: 2)" w-full />
      </n-form-item>

      <n-form-item label="Disk size" label-placement="left" label-width="150" mb-2>
        <NInputNumber v-model:value="diskSize" max="10000" min="1" placeholder="Disk size (ex: 100)" w-full />
      ...
      <n-form-item v-if="['raid_50', 'raid_60', 'raid_70'].includes(raidType)" label="Disks per stripe" label-placement="left" label-width="150" mb-2>
        <NInputNumber v-model:value="diskPerStripe" max="10000" min="2" placeholder="Number of disks per stripe (ex: 3)" w-full />
      ...
      <p>
        {{ raidInfo }}<br><br>For more information on RAID types, see<a href="https://en.wikipedia.org/wiki/Standard_RAID_levels" target="_blank" rel="noopener">Wikipedia</a>.
      </p>
    </c-card>
