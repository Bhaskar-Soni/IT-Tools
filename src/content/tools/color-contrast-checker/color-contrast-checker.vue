...
const levels = [
  { score: 3, text: "Very poor", bg: '#fbd0da', fg: '#5f071c', stars: 1 },
  { score: 4.5, text: "Poor", bg: '#fbd0da', fg: '#5f071c', stars: 2 },
  { score: 7, text: "Good", bg: '#fbf5d0', fg: '#5f5207', stars: 3 },
  { score: 12, text: "Very good", bg: '#d2fbd0', fg: '#0d5f07', stars: 4 },
  { score: 9999, text: "Super", bg: '#d2fbd0', fg: '#0d5f07', stars: 5 },
];
const smallLevels = [
...
<template>
  <div max-w-600px>
    ...
    <c-card v-if="!result.error" title="WCAG 2.1">
      <input-copyable readonly label="WCAG Contrast:" label-width="130px" label-position="left" :value="result.wcag21ConstrastValue" mb-3 />

      <n-form-item v-if="result.level" label="Contrast quality:" label-width="130px" label-placement="left">
      ...
      <n-form-item v-if="result.smallLevel" label="Small texts:" label-width="130px" label-placement="left">
      ...
      <n-form-item v-if="result.largeLevel" label="Large texts:" label-width="130px" label-placement="left">
      ...
      <c-card title="Normal text">
        <n-form-item label="WCAG 2.1 AA:" label-width="130px" label-placement="left">
          <n-tag :bordered="false" :type="result.aaNormal ? 'success' : 'error'">
            {{ result.aaNormal ? 'Pass' : 'Fail' }}
          </n-tag>
        </n-form-item>
        <n-form-item label="WCAG 2.1 AAA:" label-width="130px" label-placement="left">
          <n-tag :bordered="false" :type="result.aaaNormal ? 'success' : 'error'">
            {{ result.aaaNormal ? 'Pass' : 'Fail' }}
          </n-tag>
        </n-form-item>
      </c-card>
      <c-card title="Large text">
        <n-p><i>>-</i></n-p>
        <n-form-item label="WCAG 2.1 AA:" label-width="130px" label-placement="left">
    ...
    <c-card v-if="!result.error" title="APCA">
      <input-copyable readonly label="APCA Contrast:" label-width="130px" label-position="left" :value="result.apcaContrastValue" mb-2 />
    </c-card>
  </div>
</template>
