<template>
  <div class="script-page">
    <h2>Postgres Dump Rotation</h2>
    <p>Create compressed PostgreSQL dumps and remove old backups automatically.</p>
...
usage() {
  echo "Usage: $0 --db <database> [--backup-dir dir] [--retention-days 14]"
}
...
pg_dump "$DB_NAME" | gzip -9 > "$out"
gzip -t "$out"

echo "[INFO] Backup created and verified: $out"
echo "[INFO] Removing backups older than $RETENTION_DAYS day(s)"
...
function downloadScript() {
  ...
  a.href = url;
  a.download = `postgres-dump-rotation${scriptFormat}`;
  a.click();
