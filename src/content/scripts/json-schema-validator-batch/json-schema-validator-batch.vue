<template>
  <div class="script-page">
    <h2>JSON Schema Validator Batch</h2>
    <p>Validate entire sets of JSON files against a schema for CI and data-quality checks.</p>
...
"""
json_schema_validator_batch.py
Batch-validate JSON files against a JSON Schema.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Batch JSON Schema validator")
    ...
    for file_path in files:
        try:
            ...
            failed += 1
            print(f"[FAIL] {file_path}: {e.message}")
        except Exception as e:
            failed += 1
            print(f"[FAIL] {file_path}: {e}")

    print(f"Summary: {len(files) - failed}/{len(files)} valid")
...
function downloadScript() {
  ...
  a.href = url;
  a.download = `json-schema-validator-batch${scriptFormat}`;
  a.click();
